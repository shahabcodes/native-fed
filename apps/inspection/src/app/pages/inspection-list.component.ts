...
import { Inspection, InspectionStatus } from '@mfe-workspace/shared-services';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, ButtonComponent, LoadingComponent, InspectionCardComponent, TranslatePipe],
  template: `
    <div class="inspection-list-page">
      <header class="page-header">
        <div>
          <h1 class="page-title">{{ 'inspection.title' | translate }}</h1>
          <p class="page-subtitle">{{ 'inspection.subtitle' | translate }}</p>
        </div>
      ...
      <div class="filters-bar">
        ...
        <div class="filter-tabs">
          <button class="filter-tab" [class.active]="statusFilter() === null" (click)="setStatusFilter(null)">All ({{ totalCount() }})</button>
          <button class="filter-tab" [class.active]="statusFilter() === 'pending'" (click)="setStatusFilter('pending')">Pending ({{ pendingCount() }})</button>
          <button class="filter-tab" [class.active]="statusFilter() === 'in_progress'" (click)="setStatusFilter('in_progress')">In Progress ({{ inProgressCount() }})</button>
          <button class="filter-tab" [class.active]="statusFilter() === 'completed'" (click)="setStatusFilter('completed')">Completed ({{ completedCount() }})</button>
        </div>
      </div>
      ...
      <ng-template #loadingState><lib-loading text="Loading inspections..."></lib-loading></ng-template>
      <ng-template #emptyState>
        <div class="empty-state">
          <h3>No inspections found</h3>
          <p>Try adjusting your filters</p>
          <lib-button variant="secondary" (buttonClick)="clearFilters()">Clear Filters</lib-button>
        </div>
      </ng-template>
    </div>
  `,
  styles: [`
    ...
    .filter-tab { padding: 0.5rem 1rem; border: 1px solid #e5e7eb; border-radius: 9999px; background: white; font-size: 0.875rem; font-weight: 500; color: #6b7280; cursor: pointer; transition: all 150ms; }
...
export class InspectionListComponent implements OnInit {
  ...
  statusFilter = signal<InspectionStatus | null>(null);
  searchTerm = '';
  ...
  totalCount = computed(() => this.inspections().length);
  pendingCount = computed(() => this.inspections().filter(i => i.status === 'pending').length);
  inProgressCount = computed(() => this.inspections().filter(i => i.status === 'in_progress').length);
  completedCount = computed(() => this.inspections().filter(i => i.status === 'completed').length);

  ngOnInit(): void {
    this.inspectionApi.getInspections().subscribe({ next: (data) => { this.inspections.set(data); this.isLoading.set(false); }, error: () => this.isLoading.set(false) });
  }

  setStatusFilter(status: InspectionStatus | null): void { this.statusFilter.set(status); }
  onSearch(term: string): void { this.searchTerm = term; }
  clearFilters(): void { this.statusFilter.set(null); this.searchTerm = ''; }
  viewInspection(inspection: Inspection): void { this.router.navigate(['/inspection', inspection.id]); }
