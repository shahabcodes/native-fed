...
@Component({
  selector: 'app-in-progress-inspections',
  standalone: true,
  ...
  template: `
    <div class="inspection-list-page">
      <header class="page-header">
        <div>
          <h1 class="page-title">In Progress Inspections</h1>
          <p class="page-subtitle">Inspections currently being worked on</p>
        </div>
      ...
      <ng-template #loadingState><lib-loading text="Loading in-progress inspections..."></lib-loading></ng-template>
      <ng-template #emptyState>
        <div class="empty-state">
          <h3>No inspections in progress</h3>
          <p>Start working on a pending inspection</p>
          <lib-button variant="secondary" routerLink="/inspection/pending">View Pending Inspections</lib-button>
        </div>
      </ng-template>
    </div>
  `,
...
export class InProgressInspectionsComponent implements OnInit {
  ...
  filteredInspections = computed(() => {
    let result = this.allInspections().filter(i => i.status === 'in_progress');
    if (this.searchTerm) {
